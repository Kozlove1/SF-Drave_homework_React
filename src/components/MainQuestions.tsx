...
function MainQuestions (props) {
    ...
    return(
        <>
        <div className="drawQuestion"><Image className={"drawQuestion__img"} src={"img/undraw_questions.png"} alt={"Картинка команды"}/></div>
            <h1>Частые вопросы</h1>
                <p className="question__info">Отвечаем на вопросы, которые у вас могут возникнуть.</p>
            <section className="questions">
                <div className="questions__container">
                    {contentQuestionBox.map((content) => <Question_box question={content.question} answer={content.answer}/>)}   
                </div>
            </section>
        </>
    );
